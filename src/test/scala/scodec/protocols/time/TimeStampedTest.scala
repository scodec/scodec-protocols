  ...
  "the TimeStamped type" should {
    ...
    "support throttling a time stamped source" in {
      ...
      try {
        ...
        time(TimeStamped.throttle(source, 1.0, 10.millis).run.unsafeRunSync) shouldBe 4.seconds.toNanos +- 250.millis.toNanos
        time(TimeStamped.throttle(source, 2.0, 10.millis).run.unsafeRunSync) shouldBe 2.seconds.toNanos +- 250.millis.toNanos
      } finally {
        executor.shutdown()
      }
    }
    ...
    "support lifting a Pipe[Pure, TimeStamped[A], TimeStamped[B]] in to a Pipe[Pure, TimeStamped[Either[A, C]], TimeStamped[Either[B, C]]]" in {
      ...
      val square: Pipe[Pure, TimeStamped[Int], TimeStamped[Int]] = _.map(_.map { x => x * x })
