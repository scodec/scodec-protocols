...
class TimeSeriesTransducerTest extends ProtocolsSpec {
  ...
  "the TimeSeriesTransducer type" should {
    ...
    "support combining two transducers via an either" in {
      val add1: Transform[Unit, Int, Int] = Transform.lift(_ + 1)
      val add2: Transform[Unit, Int, Int] = Transform.lift(_ + 2)
      val x: Transform[Unit, Either[Int, Int], Int] = add1.choice(add2).xmapState(_._1)(u => (u,u))
